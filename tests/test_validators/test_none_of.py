...
from reforms.validators import NoneOf
...
def create_form(forms: Reforms) -> Callable:
    def wrapped(
        ...
        class MyForm(BaseModel):
            field: StringField(validators=[NoneOf(**args)])
...
def test_none_of_passes(test_value, test_list, create_form):
...
def test_none_of_raises(test_value, test_list, create_form):
    form = create_form(values=test_list)
    with pytest.raises(ValidationError):
        form(field=test_value)


def test_none_of_values_formatter(create_form):
